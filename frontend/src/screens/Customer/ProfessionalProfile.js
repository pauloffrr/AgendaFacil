...
export default function ProfessionalProfile({ navigation, route }) {
    ...
    const toggleStarColor = () => {
        setIconStarColor(prevColor => (prevColor === "#f1c40f" ? "#CCC" : "#f1c40f"));
    };

  return (
    <View style={styles.screen}>
        <ScrollView style={styles.container}>
            <BackButton onPress={() => navigation.navigate("Professionals Available", {id: id})}/>
            ...
            {professionalMock.filter(professional => professional.id === id).map(filteredProfessional => (
                <View key={filteredProfessional.id}> 
                    ...
                    <View style={styles.address}>
                        <FontAwesomeIcon icon={faLocationDot} size={33} style={styles.icon} />
                        <View>
                            <Text style={styles.textAddress}>{ filteredProfessional.cidade } - { filteredProfessional.estado }</Text>
                            <Text style={styles.textAddress}>{ filteredProfessional.rua }, N° { filteredProfessional.numero }</Text>
                        </View>
                    </View>

                    <Text style={styles.rayKm}>• Atende em até { filteredProfessional.raioKm }km</Text>
                    <View style={styles.favorites}>
                        <TouchableOpacity onPress={toggleStarColor}>
                            <FontAwesomeIcon icon={faStar} size={28} color={iconStarColor} />
                        </TouchableOpacity>
